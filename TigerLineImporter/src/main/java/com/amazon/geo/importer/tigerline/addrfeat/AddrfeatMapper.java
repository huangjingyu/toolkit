...
public class AddrfeatMapper extends MapReduceBase implements Mapper<NullWritable, BytesWritable, NullWritable, Text> {

    private static final NullWritable NW = NullWritable.get();
    private Text text = new Text();

    @Override
    public void map(NullWritable key, BytesWritable value, OutputCollector<NullWritable, Text> output, Reporter reporter)
            throws IOException {
    ...
    private class RecordWriter implements TLShapefileRecordWriter {
        ...
        public RecordWriter(OutputCollector<NullWritable, Text> output) {
            this.output = output;
        }
