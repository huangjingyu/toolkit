...
public class DefaultLogPuller extends AbstractLogPuller {
	...
	@Override
	public void pull(String hostName, String remoteFilePath,
			String localDestDirPath) throws Exception {
		String action = "pull " + remoteFilePath + " from " + hostName + " to "
				+ localDestDirPath;
		...
		if (success) {
			System.out.println("success to " + "pull " + remoteFilePath
					+ " from " + hostName + " to " + localDestDirPath);
		} else {
	...
	private boolean _pull(String hostName, String remoteFilePath,
			String localDestDirPath) throws Exception {
