...
public class ProgramArgs {
    ...
    public static ProgramArgs parse(CommandLine cmdLine) {
        ...
        if (StringUtils.isEmpty(t.getLogTime()) && StringUtils.isEmpty(t.getSince())
    ...
    public static Options supportOptions() {
        ...
        Option currentLogTime = OptionBuilder.withArgName("currentLogTime").hasArg(false)
                .withDescription("get log in current hour").withLongOpt("current").create("c");
        Option last = OptionBuilder.withArgName("lastHours").hasArg().withArgName("pull logs of last n hours")
                .withLongOpt("last").create();
        Option logFiles = OptionBuilder.withArgName("logFiles").hasArg()
                .withDescription("log files that you want to pull").withLongOpt("logFiles").create("f");
        Option since = OptionBuilder.withArgName("since").hasArg()
                .withDescription("pull log files since this time, inclusive").withLongOpt("since").create();
        Option until = OptionBuilder.withArgName("until").hasArg()
                .withDescription("pull log files until this time, inclusive").withLongOpt("until").create();
